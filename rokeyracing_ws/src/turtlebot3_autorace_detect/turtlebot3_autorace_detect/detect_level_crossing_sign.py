    ...
    def __init__(self):
        ...
        if self.sub_image_type == 'compressed':
            self.sub_image_original = self.create_subscription(
                CompressedImage,
                '/camera/image_raw/compressed' #'/detect/image_input/compressed'
            ...
            self.sub_image_original = self.create_subscription(
                Image,
                '/camera/image_raw/' # '/detect/image_input', # 
                ,self.cbFindTrafficSign,
                10
            )
        ...
        if self.pub_image_type == 'compressed':
            self.pub_image_traffic_sign = self.create_publisher(
                CompressedImage,
                '/camera/image_raw/compressed' #'/detect/image_output/compressed'
                , 10
            )
        elif self.pub_image_type == 'raw':
            self.pub_image_traffic_sign = self.create_publisher(
                '/camera/image_raw/' #Image, '/detect/image_output'
                , 10
            )
