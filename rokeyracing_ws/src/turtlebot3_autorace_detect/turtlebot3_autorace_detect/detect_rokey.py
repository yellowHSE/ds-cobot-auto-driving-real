...
class LaneVisualizer(tk.Frame):
    ...
    def update_image(self, cv_img, x_left=None, x_right=None, x_center=None):
        ...
        if x_left is not None:
            cv2.circle(cv_img, (x_left, 460), 6, (0, 255, 0), 5)
        if x_right is not None:
            cv2.circle(cv_img, (x_right, 460), 6, (0, 0, 255), 5)
        if x_center is not None:
            cv2.circle(cv_img, (x_center, 460), 6, (255, 0, 0), 5)

        self.hist_data.append(x_center)

        # 이미지 불러오기 (흑백)
        #img = cv2.imread(cv_img, cv2.IMREAD_GRAYSCALE)
    ...
    def update_histogram(self):
        ...
        if len(self.hist_data) > 0:
            ax.hist(self.hist_data[-50:], bins=20, color='yellow')
